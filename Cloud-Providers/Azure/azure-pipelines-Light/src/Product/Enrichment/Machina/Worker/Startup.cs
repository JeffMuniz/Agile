...
using Product.Enrichment.machina.Worker.Backend.Application.Usecases.NotifyPendingOffer;
using Product.Enrichment.machina.Worker.Backend.Domain.Services;
using Product.Enrichment.machina.Worker.Backend.Infrastructure.ExternalServices;
...
namespace Product.Enrichment.machina.Worker
{
    public class Startup : BaseWorker.Startup
    {
        ...
        public override void ConfigureServices(IServiceCollection services)
        {
            ...
            services
                .Configure<machinaConfigurationOptions>(_configuration.GetSection("machina"))
                .AddHttpClient<ImachinaService, machinaService>((provider, client) =>
                {
                    var settings = provider.GetRequiredService<IOptionsMonitor<machinaConfigurationOptions>>();
